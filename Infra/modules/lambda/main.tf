data "aws_iam_policy_document" "lambda_assume_role_1" {
  statement {
...
statement {
    effect    = "Allow"
    actions   = ["s3:GetObject","s3:ListBucket", "s3:PutObject", "rds:Connect","ec2:CreateNetworkInterface","ec2:DeleteNetworkInterface", "ec2:AttachNetworkInterface", "ec2:DetachNetworkInterface", "ec2:DescribeNetworkInterfaces", "dynamodb:PutItem"]
    resources = ["*"]
  }
}
...
resource "aws_iam_role_policy_attachment" "attach_lambda_policy_1" {
  role       = aws_iam_role.iam_for_lambda_1.name
